...
void testValueRemote() {
  for (int i = 0; i < remoteChannel; i++) { // start from 1, BATTBIND make slow, so it dn not included
    remoteValue[i] = pulseIn(remotePin[i], HIGH);
    Serial.print(remoteValue[i]); Serial.print("\t");
  }
  Serial.print("\n");
}
